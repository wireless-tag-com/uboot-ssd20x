...
int do_crccheck(cmd_tbl_t *cmdtp, int flag, int argc,
            char * const argv[])
{
#if defined(CONFIG_UPGRADE_CRC_CHECK)

    unsigned long crc_Buffer;
    uint32_t crc;
    int length = 0;
    char crc_ch[FILE_CRC_HEADER_SIZE+1];

    if (argc < 3)
        return CMD_RET_USAGE;

    crc_Buffer = simple_strtoul(argv[1], NULL, 16);

    /* Length is the number of objects, not number of bytes.
    */
    length = simple_strtoul(argv[2], NULL, 16);
    memcpy(crc_ch,(void*)(crc_Buffer+length-8),8);
    crc_ch[FILE_CRC_HEADER_SIZE] = '\0';
    crc = simple_strtoul(crc_ch,NULL,16);

    //printf("ori_crc=0x%x crc = 0x%lx \n",crc,crc32(0, crc_Buffer, length-8));

    if (crc32(0, crc_Buffer, length-8) != crc) {
        printf("crc check fail !,crc32 value not same as last 4 Bytes of the buffers \n");
        return 1;
    }
    return 0;
#else
    printf("CONFIG_UPGRADE_CRC_CHECK disable,skip CRC\n");
    return 0;
#endif
}
...
U_BOOT_CMD(
    crccheck,   3,  1,  do_crccheck,
    "crccheck address length ",
    "the last 4 Bytes was crc value,if it same as crc(buffers,length-4),it will return success"
);
