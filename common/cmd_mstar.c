...
int do_sdstar     (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    char *enable_version_chk =getenv("enable_version_chk");
    //check version,if enable_version_chk in env is 1 ,it will do version check
    if(!strcmp(enable_version_chk, "1"))
    {
        memset(cmdbuf,0,128);
        sprintf(cmdbuf, "binversioncheck mmc %s ",UpgradeImage);
        printf("cmdbuf:%s \n",cmdbuf);
    ...
    while ((next_line = get_script_next_line(&script_buf)) != NULL)
    {
        ...
        else
        {
            ...
            {
                memset(cmdbuf,0,128);
                sprintf(cmdbuf, "%ld",version);
                setenv("upgrade_bin_version",cmdbuf);
                saveenv();
                printf("Update upgrade_bin_version=%s!\n", cmdbuf);
            }
            udelay(4*1000*1000);
            run_command("reset", 0);
        }
...
int do_usbstar     (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    {
        memset(cmdbuf,0,128);
        sprintf(cmdbuf, "binversioncheck usb %s ",UpgradeImage);
        printf("cmdbuf:%s \n",cmdbuf);
        if(run_command(cmdbuf, 0) > 0)
        {
            printf("version check fail!!\n");
...
int do_binversioncheck(cmd_tbl_t *cmdtp, int flag, int argc,
            char * const argv[])
{
    char cmdbuff[128] = {0};
    char * buffer;
    ...
    sprintf(cmdbuff, "fatload %s 0 0x%x %s 0x100",argv[1],buffer,argv[2]);
    printf("cmdbuff:%s \n",cmdbuff);
    if(run_command(cmdbuff, 0) > 0)
    {
    return 1;
    }
    ...
    if(p == NULL)
    {
        printf("not find <upgrade_bin_version= \n");
        return 1;
    }
    char *p1 = strstr(p, "=");
    //printf("p1:%s \n",p1);
    ...
    memcpy(v, p1+1, p2-p1-1);
    printf("v:%s \n",v);
    version = simple_strtoul(v, NULL, 10);
    char *old_version = getenv("upgrade_bin_version");
    if(old_version != NULL)
    {
        version_env = simple_strtoul(old_version, NULL, 10);
    }
    printf(" Upgrade version=[%d]\n version from env=[%d]\n", version,version_env);

    if(version != version_env )
    {
        printf(YEL_BOLD" ->Now update version\n"RESET);
        return 0;
    }
    else
    {
        printf(YEL_BOLD" ->Already up-to-date!\n"RESET);
        return 1;
    }
}
...
U_BOOT_CMD(
        binversioncheck,	3,	1,	do_binversioncheck,
        "usbbinversioncheck filename ",
        "if version in usbbin != version in env ,return 0 else return 1"
);
