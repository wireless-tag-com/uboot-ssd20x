...
//Select SPI Read Command interface //move to menuconfig
...
#if defined CONFIG_MS_SPINOR_RIUISP
    SPI_READ_MODE gReadMode = E_RIUISP_MODE;
#elif defined CONFIG_MS_SPINOR_BURST_READ
    SPI_READ_MODE gReadMode = E_FAST_MODE;
#elif defined CONFIG_MS_SPINOR_NORMAL_READ
    SPI_READ_MODE gReadMode = E_SINGLE_MODE;
#elif defined CONFIG_MS_SPINOR_DUAL_READ
    SPI_READ_MODE gReadMode = E_DUAL_AD_MODE;
#elif defined CONFIG_MS_SPINOR_QUAL_READ
...
MS_BOOL HAL_SERFLASH_DetectType(void)
{
   ...
   // MS_U8   u8Status0, u8Status1;
    ...
    if (gReadMode==E_QUAD_MODE && _hal_SERFLASH.u8MID!=MID_MXIC && _hal_SERFLASH.u8MID!=MID_WB && _hal_SERFLASH.u8MID!=MID_ZB)
    {
    ...
    if (_hal_SERFLASH.u8MID == MID_WB)
    {
        ...
        HAL_FSP_WriteStatus(SPI_CMD_WRSR2, 1, &u8data);
    }
...
{
    BOOL bRet = FALSE;
    MS_U8 u8EAR = u32Offset >> 24; // address bit 25:31
    U32 u32DstAddr = (U32)pu8Data - 0x20000000;

    if((u8EAR == 0) && ((u32Offset + u32Size) > 0x1000000))
    {
    ...
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr+(0x00<<2), 0x0000);
    //5:source device (spi)
    //3:source device data width (8 bytes)
    //0:destination device (channel 0)
    //4:destination device data width (16 bytes)
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x02<<2), 0x4035);

    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x03<<2), 0x0000); //0:channel0: MIU0

    // Set source address
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x04<<2), (U16)(u32Offset & 0xFFFF));
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x05<<2), (U16)(u32Offset >> 16));

    // Set destination  address
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x06<<2), (U16)(u32DstAddr & 0x0000FFFF));
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x07<<2), (U16)(u32DstAddr >> 16));
    // Set Size
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x08<<2), (U16)(u32Size & 0xFFFF));
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x09<<2), (U16)(u32Size >> 16));

    // Trigger
    WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x00<<2), 1);

    do
    {
        //check done
        if(READ_WORD(_hal_isp.u32BdmaBaseAddr + (0x01<<2)) & 0x08)
        {
            //clear done
            WRITE_WORD(_hal_isp.u32BdmaBaseAddr + (0x01<<2), 0x08);
            bRet = TRUE;
            break;
        }
    } while(1);
...
MS_BOOL _HAL_QP_BDMAToFSP(MS_U32 u32Src_off, MS_U32 u32Dst_off, MS_U32 u32Len)
{
   ...
   // printk("    %s(0x%08X, %3d)\n", __FUNCTION__, (int)u32Dst_off, (int)u32Len);
    ...
    WRITE_WORD((_hal_isp.u32BdmaBaseAddr + (0x5<<2)), (u32DMAAddr >> 16));
    // Set end address
...
#if  defined(CONFIG_MS_SPINOR_QUAL_WRITE)
