...
void _HalPnlIfSetMipiDsiParamConfig(void *pCtx, void *pCfg)
{
    ...
    PNL_DBG(PNL_DBG_LEVEL_HAL, "[HALPNL]%s %d:: Htt:%d, Vtt:%d Fps:%d, ScPixClk:%ld, PolCh(%d %d %d %d %d), Packet:%s, HSpeedCmd:%d\n",
        __FUNCTION__, __LINE__,
    ...
    if(pstMipiDsiCfg->pu8CmdBuf && pstMipiDsiCfg->u32CmdBufSize !=0)
    {
        ...
        if(pstMipiDsiCfg->u8HSpeedCmdMode == 0)
        {
