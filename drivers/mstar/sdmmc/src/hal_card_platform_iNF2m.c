...
void Hal_CARD_IPOnceSetting(IPEmType eIP)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        ...
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,REG_CLK_SDIO0), BIT05_T|BIT04_T|BIT03_T|BIT02_T|BIT01_T|BIT00_T); //[4:2]: Clk_Sel: 0:48MHz [1]: Clk_i [0]: Clk_g
...
void Hal_CARD_PullPADPin(PADEmType ePAD, PinPullEmType ePinPull, BOOL_T bTwoCard)
{
    if(ePAD == EV_PAD1) //PAD_SD
    {
        if(ePinPull ==EV_PULLDOWN)
        {
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CHIPTOP_BANK, 0x08), BIT08_T | BIT09_T);  //sdio mode = 0
                                                                                       //
...
void Hal_CARD_PowerOn(PADEmType ePAD, U16_T u16DelayMs)
{
    if(ePAD==EV_PAD1) //PAD_SD
    {
        ...
        #ifdef CONFIG_SDMMC_PWR_VALID_H
