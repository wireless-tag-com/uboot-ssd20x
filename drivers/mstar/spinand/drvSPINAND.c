...
//                memcpy(&_gtSpinandInfo, &gtSpiNandInfoTable[u32Index], sizeof(SPINAND_FLASH_INFO_t));
    ...
    if (pSpiNandDrv->tSpinandInfo.au8_ID[0] == 0xCD && pSpiNandDrv->tSpinandInfo.au8_ID[1] == 0x71) /*FORESEE NAND flash need to pull up QE bit*/
    {
#if defined(SUPPORT_SPINAND_QUAD) || SUPPORT_SPINAND_QUAD
    ...
    if (pSpiNandDrv->tSpinandInfo.au8_ID[0] == 0x01 &&
        (pSpiNandDrv->tSpinandInfo.au8_ID[1] == 0x25 || pSpiNandDrv->tSpinandInfo.au8_ID[1] == 0x35)) /*S35ML NAND flash need setting A[0] 1BIT to unlock A[0]reg*/
    {
        U8 u8Status;
