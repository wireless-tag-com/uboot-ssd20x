...
if [ "${chip}" = "i2m" ]; then
  ...
  make infinity2m_nor_emmc_defconfig
  make clean; make
  if [ ! -d $alkaid_dir/project/board/${chip}/boot/emmc/uboot ]; then
    mkdir $alkaid_dir/project/board/${chip}/boot/emmc/uboot
  fi
  cp $uboot_dir/u-boot.xz.img.bin $alkaid_dir/project/board/${chip}/boot/emmc/uboot/u-boot.xz.img.bin

  make infinity2m_spinand_emmc_defconfig
  make clean; make
  if [ ! -d $alkaid_dir/project/board/${chip}/boot/emmc/uboot ]; then
    mkdir $alkaid_dir/project/board/${chip}/boot/emmc/uboot
  fi
